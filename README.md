...
need the program `pre_mr1` which can be executed with
...
To compute the subsumption relationships across all tests using the
unmodified Papadakis algorithm. It might be convenient to
...
* `06-list-subsuming.py` lists the subsuming mutants present in one of the `V1*0.yaml` files generated by the `01-subsumption.py` script.
